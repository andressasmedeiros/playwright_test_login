...
test("the user insert wrong credentials", async ({ page }) => {
    ...
    await page.locator('[data-test="password"]').fill('secret_invalid');
    await page.locator('[data-test="login-button"]').click();
    const errorText = await page.getByText('Epic sadface: Username and passqord do not match any user in this service');
